...
void transposeMatrix(int mat[][MAX_SIZE], int rows, int cols, int result[][MAX_SIZE])
{
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
...
int main()
{
    ...
    if (choice == 1)
    {
        // Input matrix 2 for multiplication
    ...
    else if (choice == 3)
    {
        ...
        displayMatrix(cols1, rows1, result); // Note: rows and columns are swapped for the transpose
    }
    else
    {
